...
class MongoNewsletterSubscriberRepository {
  ...
  async createCampaign(payload) {
    const doc = await this.NewsletterCampaignModel.create({
      ...
      deliveredCount: payload.deliveredCount ?? payload.sentCount ?? 0,
      sentCount: payload.sentCount ?? payload.deliveredCount ?? 0,
  ...
  serializeCampaign(doc) {
    return {
      ...
      deliveredCount: doc.deliveredCount ?? doc.sentCount ?? 0,
      sentCount: doc.sentCount ?? doc.deliveredCount ?? 0,
